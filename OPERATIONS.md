...
> **最終更新:** 独自機能を「探す/決めるラボ」「買う前チェック」「ウォッチリスト司令室」に再編し、ビルド・Vitest・Playwright を通した最新版
...
| `/discover` | **探す/決めるラボ**。気分・予算・セール状況から候補を出す | ビルド毎 |
| `/buy-timing` | **買う前チェック**。買い時、まとめ買い、次のセール波を確認 | ビルド毎 |
| `/watchlist` | **ウォッチリスト司令室**。保存作品、値下げ候補、次候補を管理 | リアルタイム |
| `/personalized` | ウォッチリスト起点の自分向けフィード | リアルタイム |
...
1. **探す/決めるラボ (`/discover`)**  
   SNSや検索流入で来たユーザーが、気分・予算・セール状況から最初の候補を絞る入口。
2. **買う前チェック (`/buy-timing`)**  
   価格、割引率、次のセール時期、まとめ買い可否を見て離脱を減らす判断ページ。
3. **ウォッチリスト司令室 (`/watchlist`)**  
   再訪ユーザー向け。保存作品の値下げ確認と次候補の発見をまとめる中核。
4. **ランキング / セール / 週間セール**  
   検索需要を拾う定番導線。情報収集目的のユーザーを流し込みやすい。
5. **ガイド / 比較記事**  
   新規登録と初回購入の導線。SNS固定ポストやプロフィールリンクと相性が良い。
...
2. **探す/決めるラボ**で「今日は何を見るか」を絞る
3. **買う前チェック**で価格・割引率・まとめ買い候補を比較する
4. **ウォッチリスト司令室**に保存して次回の再訪導線を作る
...
### 運用側の見方
- ホームの訴求で強く出すのは **探す/決めるラボ / 買う前チェック / ウォッチリスト司令室**
