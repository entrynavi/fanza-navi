...
import {
  ...
  FaClock, FaShoppingCart, FaListOl, FaChartBar,
  FaHistory, FaShareAlt, FaUser, FaRandom, FaPen, FaProjectDiagram,
} from "react-icons/fa";
...
/* ------------------------------------------------------------------ */
/*  Feature Categories — consolidate 21+ tools into clear groups        */
/* ------------------------------------------------------------------ */
...
const toolCategories: ToolCategory[] = [
    ...
    tools: [
      {
        href: ROUTES.search,
        title: "作品検索",
        description: "21万件超のFANZA作品をブラウザ内で高速検索。価格・評価・ジャンルで絞り込み。",
      ...
      },
      {
        href: ROUTES.gacha,
        title: "ガチャレコメンド",
        description: "21万件超から条件を決めて回す、サプライズ発見機能です。",
        icon: <FaDice size={20} />,
        badge: "21万件",
      },
      {
  ...
  },
  {
    id: "sale",
    label: "セール・買い時",
    description: "値下げ情報と購入タイミングを判断する",
    icon: <FaTags size={16} />,
    color: "#d3af6f",
    tools: [
      {
...
const allUniqueTools = toolCategories.flatMap((cat) => cat.tools);
    ...
    <main className="pb-12">
      ...
      <section className="content-shell px-4 pb-10">
        <SectionIntro
          ...
          description={`探す・比べる・買い時判断・管理共有の4カテゴリ、計${allUniqueTools.length}種の独自機能をそろえています。`}
        />

        <div className="space-y-6">
          {toolCategories.map((category) => {
            const visibleCategoryTools = showAllTools ? category.tools : category.tools.slice(0, 3);
            return (
            <div key={category.id}>
              {/* Category header */}
        ...
        {!showAllTools && (
          <div className="mt-5 text-center">
            ...
            >
              全{allUniqueTools.length}機能（{toolCategories.length}カテゴリ）を表示 <FaArrowRight size={10} />
            </button>
          </div>
        )}
      </section>
